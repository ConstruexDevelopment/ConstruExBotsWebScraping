...
def extraer_direccion(link, id_exhibidor):
    try:
        ...
        if direcciones:
            ...
            # Guardar enlaces sin direcciones en otro archivo
            with open('sin_direcciones.csv', 'a', newline='', encoding='utf-8') as csvfile:
...
# Leer enlaces desde un archivo CSV utf-8
...
with open('sin_direcciones.csv', 'w', newline='', encoding='utf-8') as csvfile:
    fieldnames = ['ID_exhibidor', 'Enlace']
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    writer.writeheader()
